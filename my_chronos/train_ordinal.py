...
def train():
    with open('ordinal_config_fixed.yaml', 'r') as f:
        config = yaml.safe_load(f)
    
    config['epochs'] = 100
    config['learning_rate'] = 0.001
    config['batch_size'] = 4
    config['max_series'] = 100
    config['n_tokens'] = 66
    
    print("="*60)
    print("TRAINING WITH MSE LOSS AND 64 BINS")
    print("="*60)
    ...
    try:
        ...
        import pandas as pd
        df = pd.read_parquet("./data/m4_hourly_train.parquet")
        print("Loaded from local file")
    
    train_series = []
    required_len = config['context_length'] + config['prediction_length']
    
    for series_id in df['id'].unique()[:config['max_series']]:
    ...
    model.train()
    best_loss = float('inf')
    output_dir = Path("./ordinal_model_checkpoints_final")
